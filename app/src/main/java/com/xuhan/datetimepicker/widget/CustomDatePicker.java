package com.xuhan.datetimepicker.widget;
...
import com.xuhan.datetimepicker.R;
...
public class CustomDatePicker {
    ...
    private TextView tv_cancle, tv_select, year_text, month_text, day_text, hour_text, minute_text;
    private boolean showHourAndMin;
    ...
    private void initView() {
        ...
        tv_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (showHourAndMin && showDate) {
                    handler.handle(time);
                } else if(!showHourAndMin && showDate){
                    handler.handle(time.split(" ")[0]);
                }else if(showHourAndMin && !showDate){
                    handler.handle(time.split(" ")[1]);
                }
                datePickerDialog.dismiss();
            }
        });
    ...
    private void initTimer() {
        ...
        if (spanYear) {
            ...
            if ((scrollUnits & SCROLL_TYPE.MINUTE.value) != SCROLL_TYPE.MINUTE.value) {
                minute.add(formatMinUnit(startMinute));
            } else {
                for (int i = startMinute; i <= MAX_MINUTE; i++) {
                    minute.add(formatMinUnit(i));
                }
            }
        } else if (spanMon) {
    ...
    private String formatMinUnit(int unit){
        return unit%2 != 0 ? "00" : "30";
    }
